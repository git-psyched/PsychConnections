...
function App() {
  ...
  const Layout = (children) => e('div', {className: 'min-h-screen bg-slate-950 text-white flex justify-center items-start overflow-x-hidden'},
    e('div', {className: 'w-full max-w-md p-4 flex flex-col min-h-screen'}, children)
  );

  if (view === 'help') return Layout(
    e('div', {className: 'flex-grow flex flex-col justify-center'},
      e('div', {className: 'glass p-8 rounded-[40px] border-white/10 text-center'},
    ...
    e('div', {className: 'grid grid-cols-1 gap-3 pb-10'},
      [...Array(30)].map((_, i) => {
        ...
        return e('button', {key: ds, onClick: () => startPuzzle(ds), className: `flex justify-between items-center p-5 rounded-3xl border transition-all ${history[ds] ? 'bg-emerald-900/20 border-emerald-500/30' : 'bg-slate-900/40 border-white/5'}`}, 
          ...
          history[ds] ? e('span', {className: 'text-[10px] text-emerald-500 font-black'}, "SOLVED") : e('span', {className: 'text-[10px] text-slate-600 font-black'}, "LOCKED")
        );
      })
    )
  ]);
  ...
  if (view === 'game') return Layout([
    e('header', {className: 'flex justify-between items-center mb-6 bg-slate-900/40 p-4 rounded-2xl border border-white/5'},
    ...
    e('div', {className: 'mt-auto pb-10 flex flex-col items-center gap-4'},
      ...
      (mistakes >= 4 || foundGroups.length === 4) ? 
        ...
        e('button', {onClick: submit, disabled: selected.length !== 4, className: 'w-full py-5 bg-white text-slate-950 font-black rounded-[24px] disabled:opacity-20 shadow-xl uppercase text-sm'}, 'Verify Sequence')
    ),
    showDetail && e('div', {className: 'fixed inset-0 z-50 flex items-center justify-center p-6 bg-slate-950/95 backdrop-blur-md'},
      e('div', {className: 'glass p-8 rounded-[40px] w-full max-w-sm border border-white/20 shadow-2xl transform scale-100'},
        e('h2', {className: 'text-2xl font-black mb-6 text-white text-center font-mono uppercase'}, showDetail.name),
        e('div', {className: 'space-y-4 mb-8'},
          showDetail.clues.map((c, idx) => e('div', {key: idx, className: 'flex items-center gap-4 p-3 rounded-2xl bg-white/5'},
            e('div', {className: `w-3 h-3 rounded-full ${CATEGORY_COLORS[c.category].bg} shadow-[0_0_8px_currentColor]`}),
            e('div', null, e('div', {className: 'text-[8px] uppercase text-slate-500 font-black'}, c.category), e('div', {className: 'text-xs text-slate-100 font-bold font-sans'}, c.text))
          ))
        ),
